...
export default function EditProfilePage() {
  const currentUserId = "3c780894-f227-4639-894b-7f6355e1631d"; 
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const { data } = await axios.get(`/api/profile/get?userId=${currentUserId}`);
        if (data.success && data.data) {
          setFormData({
            memberId: data.data.member_id || "N/A",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await axios.post('/api/profile/update', { userId: currentUserId, ...formData });
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 p-6 bg-white rounded-lg shadow-md border border-gray-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white p-3 mt-4 rounded hover:bg-blue-700">
