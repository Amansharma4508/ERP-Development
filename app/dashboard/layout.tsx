...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  const { user, logout, isLoading, walletOnboardingStatus } = useAuth();
  ...
  useEffect(() => {
    ...
    // Redirect virtual wallet users to their portal
    if (user.role === 'wallet_user') {
      router.push('/virtual-wallet');
      return;
    }

    // ✅ FLOW CHECK: Agar user role 'user' (Patient) hai aur usne wallet form fill nahi kiya 
    // (status 'pending' ya 'none' hai), toh use zabardasti /onboarding/wallet par bhej do!
    if (user.role === 'user' && (walletOnboardingStatus === 'pending' || walletOnboardingStatus === 'none')) {
      router.replace('/onboarding/wallet');
      return;
    }
  }, [user, isLoading, walletOnboardingStatus, router]);
        ...
        <header className="h-16 bg-card border-b border-border flex items-center justify-between px-4 lg:px-8 sticky top-0 z-20 w-full flex-shrink-0">
          ...
          <div className="flex items-center gap-4 shrink-0 ml-auto" ref={dropdownRef}>
            ...
            <div className="relative">
              ...
              >
                {userAvatar ? (
                  <img src={userAvatar} alt={user.fullName || 'User'} className="w-8 h-8 rounded-xl object-cover shrink-0 ring-2 ring-indigo-500/20" />
                ) : (
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 rounded-2xl border border-border bg-card p-1.5 shadow-xl animate-in fade-in slide-in-from-top-2 duration-150 z-50">
                  ...
                  >
                    <Settings size5={14} className="text-muted-foreground" />
                    Edit Profile
                  </Link>
