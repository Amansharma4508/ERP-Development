...
// Only these roles can be self-selected from the public register form.
// 'admin' is intentionally excluded — admin accounts are invite-only.
const ALLOWED_SELF_REGISTER_ROLES = ['user', 'doctor', 'logistics', 'wallet_user'];

// Roles that need an admin to flip is_approved to true before they can use their dashboard
const NEEDS_APPROVAL = ['doctor', 'logistics'];

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    let { email, password, fullName, role = 'user' } = body;

    if (!email || !password || !fullName) {
      return toJson(errorResponse('All fields are required', 400));
    }

    // Hard block: whatever the client sends, if it's not in the whitelist
    // (this silently blocks 'admin' too), force it down to 'user'.
